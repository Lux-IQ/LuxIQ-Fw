...
program
  .name('luxiq')
  .description('LuxIQ Framework - Create, list, and interact with AI agents via CLI')
  .version('1.0.0');
...
function displayWelcome() {
  console.log('\n┌─────────────────────────────────────────────────┐');
  console.log('│            Welcome to LuxIQ AI                  │');
...
function showCommandHelp(command: string) {
  switch(command) {
    ...
    case 'list':
      console.log('\n📝 Usage: list');
      console.log('This command lists all available agents in the LuxIQ Network.');
...
async function main() {
  try {
    ...
    console.log('Connecting to LuxIQ API...');
    const isConnected = await ApiService.checkApiConnectivity();
    
    if (!isConnected) {
      console.error('\n❌ Cannot connect to the LuxIQ API.');
      console.error('Please check:');
      console.error('  - Your internet connection');
      console.error(`  - If the LuxIQ API server is running`);
      ...
      if (!retryConnection) {
        console.error('\n❌ Still unable to connect to the LuxIQ API.');
        ...
        console.log('\n✅ Connected to LuxIQ API successfully!\n');
      }
    } else {
      console.log('✅ Connected to LuxIQ API successfully!\n');
    }
    ...
    if (error instanceof Error) {
      ...
      if (error.message.includes('required but not set in environment variables')) {
        ...
        console.error('API_BASE_URL=https://luxiq-api.vercel.app');
      }
    } else {
      console.error('An unexpected error occurred.');
    }
