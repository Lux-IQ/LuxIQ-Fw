...
// Create axios instance for the LuxIQ API
...
function handleApiError(error: unknown, operation: string): Error {
  if (axios.isAxiosError(error)) {
    ...
    if (error.code === 'ECONNREFUSED' || error.code === 'ECONNRESET' || error.code === 'ECONNABORTED' || !error.response) {
      return new Error(`Cannot connect to the LuxIQ API. Please check your internet connection and try again later.`);
    }
    ...
    if (error.response) {
      ...
      if (error.response.status === 404) {
        ...
        return new Error(`The LuxIQ API server is currently experiencing issues. Please try again later.`);
      }
...
/**
 * Service for communicating with the LuxIQ API
 */
